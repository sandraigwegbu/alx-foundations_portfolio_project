...
def home():
    if request.method == "POST":
        ...
        tz_json = timeZone(latitude, longitude, dateString, timeString) # resume
        #tz = tz_json["rawOffset"]/3600
        tz = (tz_json["rawOffset"] + tz_json["dstOffset"])/3600  # corrected for dst
        timeZoneId = tz_json["timeZoneId"]
        timeZoneName = tz_json["timeZoneName"]
        timeZoneInfo = (f"UTC+{tz}: {timeZoneId}") if (tz > 0) else (f"UTC{tz}: {timeZoneId}")
        ...
        cosPhi = panelSunZenithCosine(panelAzimuth, panelSlope, sunAzimuth, sunElevation)
        solarGeo = (round(cosPhi, 2)) if (cosPhi > 0) else ("#NA (panel in shaddow)")
        ...
        return render_template("solargeometry_form_and_session.html", \
            ...
            panelAzimuth=panelAzimuth, dateString=dateString, timeString=timeString, \
                timeZoneInfo=timeZoneInfo, lat=round(latitude, 6),
...
def timeZone(lat, lng, dateString, timeString):
    ...
    timeStamp = datetime.fromisoformat(dateString + "T" + timeString).timestamp()
...
def noaa(latitude, longitude, dateString, timeString):
    ...
    geomMeanLongSun = fmod2(280.46646 + julianCentury * (36000.76983 + julianCentury * 0.0003032),360)
    ...
    obliqCorr = meanObliqEcliptic + 0.00256 * cos(radians(125.04 - 1934.136 * \
        julianCentury))
    sunRtAscen = degrees(atan2_excel(cos(radians(sunAppLong)), cos(radians(obliqCorr)) * sin(radians(sunAppLong))))
    ...
    solarNoon = (720.0 - 4.0 * longitude - eqOfTime + float(tz) * 60.0) / 1440.0 + dstOffset / 86400.0
    ...
    trueSolarTime =fmod2(tm * 1440 + eqOfTime + 4 * longitude - 60 * float(tz), 1440)
    ...
    print(dt, tm, julianDay, julianCentury, geomMeanLongSun, geomMeanAnomSun, eccentEarthOrbit, sunEqOfCtr)
    ...
    print(haSunrise, fracDaytoTime(solarNoon), fracDaytoTime(sunriseTime), fracDaytoTime(sunsetTime))
    print(sunlightDuration, trueSolarTime, hourAngle)
    print(solarZenithAngle, solarElevAngle, approxAtmRefraction, solarElevCorrForAtmRefraction, solarAzimuthAngle)
    return (solarAzimuthAngle, solarElevAngle, sunlightDuration / 60, fracDaytoTime(solarNoon, seconds=False))  # (170.3, 45.1, 5.2, '12:08')
...
def panelSunZenithCosine(panelAzimuth, panelSlope, sunAzimuth, sunElevation):
    ...
    # r1 = r2 = 1:
    deltaVertProjection = sin(radians(90 - int(panelSlope))) - sin(radians(sunElevation))
